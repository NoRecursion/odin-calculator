...
function lexToFields(inp,field){return getFields(lex.lexer(inp),field)};
...
describe('Correctly locates positions', () => {
  test('simple whitespaces', () => {
    ...
    const lexOutput = lex.lexer(lexInput)[0].position;
    expect(lexOutput).toStrictEqual(expectedOutput);
  });
  test('whitespaces and tokens', () => {
    ...
    const lexOutput = lex.lexer(lexInput)[1].position;
    expect(lexOutput).toStrictEqual(expectedOutput);
  });
  test('complex input', () => {
    ...
    const lexOutput = lex.lexer(lexInput).filter(token=>token.value==']')[1].position;
    expect(lexOutput).toStrictEqual(expectedOutput);
  });
});
